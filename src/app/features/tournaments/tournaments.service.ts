...
import type { Battler, BattleMove, BattleRules } from '../../game/engine';
import {
  buildBracket,
  levelForRound,
  modeById,
...
export class TournamentService {
  ...
  readonly champion = computed(() => this.bracket()?.champion ?? null);
  ...
  /** Start a tournament in the given mode (draft modes pause for picks). */
  async startMode(modeId: ModeId): Promise<void> {
    ...
    try {
      ...
      this.bracket.set(buildBracket(trainers));
  ...
  private runAiMatches(): void {
    ...
    this.bracket.set(b);
    if (b.champion) this.status.set('done');
  }
